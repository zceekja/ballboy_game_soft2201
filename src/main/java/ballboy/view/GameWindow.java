...
public class GameWindow {
    ...
    private void draw() {
        model.tick();
        if(model.getCurrentLevel().isGameEnd() ==1){
            gc.fillText("GAME FINISHED",250,200);
            return;
        }
        ...
        gc.fillText(model.getCurrentLevel().getObservers().get(0).print(),20,50);
        gc.fillText(model.getObservers().get(0).print(),400,50);
